...
def collectAnim(context, armature, scale, bones, action, startframe = None, endframe = None, useKeyFrames = False):
	...
	# JOHN'S CODE to get just keyframes, rather than all between start/end
	frameTimes = set()
	if (useKeyFrames):
		# Walk keyframe points, add keyframe times
		for FC in action.fcurves:
			for KF in FC.keyframe_points:
				kfTime = int(KF.co[0])
				frameTimes.add(kfTime)
	else:
		# add every time between start/endframe (including endframe)
		frameTimes = {t for t in range (startframe, endframe+1)}

	# iterate through the times you want
